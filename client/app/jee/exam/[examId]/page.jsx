...
const JeeExam = () => {
  ...
  const saveAndNext = () => {
    if (selectedOption) {
      ...
      setAnswers((prev) => ({ ...prev, [subject[0] + "-" + currentQuestionIndex]: selectedOption }))
    } else {
      setQuestionIndexes(prevIndexes => ({
        ...prevIndexes,
        [subject]: prevIndexes[subject].map((item, i) =>
  ...
  const setOption = (optionNumber) => {
    ...
    inputs.forEach((option, i) => {
      option.checked = i === optionNumber - 1 ? true : false
    })
  }
  ...
  return (
    <MathJaxContext>
      {loading && <div className="fixed top-0 left-0 h-screen w-full bg-white z-[100] flex flex-col  items-center justify-center "> <h1 className="text-blue-700 text-4xl font-bold">ExamMocker</h1><br /><h1 className="text-2xl font-semibold text-orange-500">Loading...</h1> </div>}
      <div className='max-h-max min-h-screen w-full bg-white text-black'>
        <div className=" text-blue-700 font-bold justify-between px-3 pb-1 items-center flex  h-16">
          ...
          <div>
            <h1 className="text-lg">Candidate Name : Aban Muhammed C P</h1>
            <h4 className="flex text-lg  items-center">Remaining Time : <div className="text-white justify-center  bg-blue-600 ml-1 w-24  font-[400] text-lg flex items-center h-[18px] px-2 rounded-full ">{String(time.hours).length == 1 ? "0" + time.hours : time.hours}:{String(time.minutes).length == 1 ? "0" + time.minutes : time.minutes}:{String(time.seconds).length == 1 ? "0" + time.seconds : time.seconds}</div></h4>
          </div>
        </div>
          ...
          <div className="w-[70%]">
            ...
            <div className="h-[23rem] border-b border-black overflow-y-scroll">
              ...
              <div className="flex flex-col gap-3 p-3 ">
                <label htmlFor="mcq_option_1" className="w-fit cursor-pointer flex gap-3"><input onChange={handleOptionChange} className="cursor-pointer option-input " type="radio" name="mcq_option" id="mcq_option_1" value="A" /><MathJax><h1 className="text-xl font-medium"> (1) <span className="ml-5">{currentQuestion?.options[0]}</span></h1></MathJax></label>
                <label htmlFor="mcq_option_2" className="w-fit cursor-pointer flex gap-3"><input onChange={handleOptionChange} className="cursor-pointer option-input " type="radio" name="mcq_option" id="mcq_option_2" value="B" /><MathJax><h1 className="text-xl font-medium"> (2) <span className="ml-5">{currentQuestion?.options[1]}</span></h1></MathJax></label>
                <label htmlFor="mcq_option_3" className="w-fit cursor-pointer flex gap-3"><input onChange={handleOptionChange} className="cursor-pointer option-input" type="radio" name="mcq_option" id="mcq_option_3" value="C" /><MathJax><h1 className="text-xl font-medium"> (3) <span className="ml-5">{currentQuestion?.options[2]}</span></h1></MathJax></label>
                <label htmlFor="mcq_option_4" className="w-fit cursor-pointer flex gap-3"><input onChange={handleOptionChange} className="cursor-pointer option-input " type="radio" name="mcq_option" id="mcq_option_4" value="D" /><MathJax><h1 className="text-xl font-medium"> (4) <span className="ml-5">{currentQuestion?.options[3]}</span></h1></MathJax></label>
              </div>
            </div>
